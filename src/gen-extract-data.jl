...
function loadexample(data_choice)
    ...
    if data_choice=="generated"
        ...
        λmax = λ(0.0)
        observations = samplepoisson(λn, n*λmax, T,n)   # sample ppp on [0,T] with intensity function λ
    ...
    elseif data_choice=="testdat_n1"
        n = 1
        observations = vec(readdlm(datadep"testdat_n1.csv"))
        T = 10.0
    ...
    elseif  data_choice=="testdat_n5"
        n = 5
        observations = vec(readdlm(datadep"testdat_n5.csv"))
        T = 10.0
        λ = function(x::Float64)
             2* (5 + 4*cos(x)) * exp(-x/5)
        end
    ...
    elseif  data_choice=="testdat_n4000"
        n = 4000
        observations = vec(readdlm(datadep"testdat_n4000.csv"))
        T = 10.0
        λ = function(x::Float64)
             2* (5 + 4*cos(x)) * exp(-x/5)
        end
    ...
    elseif  data_choice=="simpson_n200"
        n = 200
        observations = vec(readdlm(datadep"simpson_n200.csv"))
        T = 6.0
        λ = function(x::Float64)
    ...
    elseif     data_choice=="simpson_n500"
        n = 500
        observations = vec(readdlm(datadep"simpson_n500.csv"))
        T = 6.0
        λ = function(x::Float64)
            y = 0.5 * pdf(Normal(3,1), x)
            for j=0:4
    ...
    elseif   data_choice=="unit_intensity"
        observations = vec(readdlm(datadep"unit_intensity.csv"))
        n = 1
        T = 50.0
        λ = function(x::Float64)
            1.0
